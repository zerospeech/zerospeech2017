...
dnl $Header: /u/drspeech/repos/feacalc/aclocal.m4,v 1.3 2012/01/05 20:30:15 dpwe Exp $
dnl
dnl Clients:
...
AC_DEFUN(ACDPWE_PROG_CC_CXX, [
  ...
  if test -n "$dpwe_doCxx" ; then 
    AC_MSG_CHECKING([default C++ compiler])
    AC_ARG_WITH(cxx, [  --with-cxx=CXX         set C++ compiler flags to CXX],
        [CXX="$with_cxx"])
    AC_MSG_RESULT([$CXX])
    AC_MSG_CHECKING([default C++ compiler flags])
    if test -z "$CXXFLAGS" ; then
	CXXFLAGS="-O -D_FILE_OFFSET_BITS=64"
    fi
