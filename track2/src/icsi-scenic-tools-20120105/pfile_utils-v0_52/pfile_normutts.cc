/*
    ...
    *within each utterance* to have zero mean and unit variance.  
    ...
    $Header: /u/drspeech/repos/pfile_utils/pfile_normutts.cc,v 1.2 2012/01/05 20:32:02 dpwe Exp $
...
static void
pfile_normutts(QN_InFtrLabStream& in_stream,
	       ...
	       int verbose)
{
	...
	if (donorm) {
		...
		fprintf(stdout, "Means/SDs for utt %d:\n", 
			(*srit));
		fprintf(stdout, "Mean:   ");
		for (i = 0; i < (int)frrng.length(); ++i) {
		    fprintf(stdout, " %7.3f", mean_sd_buf[2*i]);
		}
		fprintf(stdout, "\n  SD:   ");
		for (i = 0; i < (int)frrng.length(); ++i) {
		    fprintf(stdout, " %7.3f", mean_sd_buf[2*i+1]);
		}
		fprintf(stdout, "\n");
	    }
	}
