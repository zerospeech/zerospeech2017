// $Header: /u/drspeech/repos/quicknet2/testsuite/onlftr_test2.cc,v 1.2 2004/09/16 00:13:50 davidj Exp $
//
// Various tests for onlftr classes
...
void
create_file(int debug, const char* filename)
{

    size_t i;			// Counter
    FILE* lf;			// The onlftr file
    float ftrs[num_ftrs*2];	// A buffer for features (two rows worth)

    lf = QN_open(filename, "w");
    QN_OutFtrStream_OnlFtr* ostrp =
	new QN_OutFtrStream_OnlFtr(debug, "file", lf, num_ftrs);
...
void
check_file(int debug, const char* filename)
{
    size_t i;			// Counter
    FILE* lf;			// The onlftr file
    ...
    QN_InFtrLabStream_OnlFtr* istrp =
	new QN_InFtrLabStream_OnlFtr(debug, "file", lf, num_ftrs, 1);
...
main(int argc, char* argv[])
{
    ...
    assert(arg==argc-1);
    // Usage: onlftr_test2 <file>
    const char* file = argv[arg++];
    if (rtst_logfile!=NULL)
	debug = 99;
    onlftr_wr_test(debug, file);
