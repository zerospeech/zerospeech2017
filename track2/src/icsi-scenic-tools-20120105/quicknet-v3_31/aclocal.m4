...
AC_ARG_WITH(cflags, [  --with-cflags=FLAGS     set C compiler flags to FLAGS],
    [CFLAGS="$with_cflags"])
AC_MSG_RESULT([$CFLAGS])
])

dnl ****************************************************************
dnl A multiple name version of AC_CHECK_TOOL
dnl ****************************************************************

dnl ACSPERT_CHECK_TOOLS(VARIABLE, TOOLS-TO-CHECK-FOR [, VALUE-IF-NOT-FOUND])
AC_DEFUN(ACSPERT_CHECK_TOOLS,
[AC_REQUIRE([AC_CHECK_TOOL_PREFIX])dnl
for ac_prog in $2
do
  AC_CHECK_PROG($1, ${ac_tool_prefix}[$]ac_prog, ${ac_tool_prefix}[$]ac_prog, )
  test -n "[$]$1" && break
done
if test -z "[$]$1"
then
...
dnl ****************************************************************
dnl A combination of AC_CHECK_TOOLS and AC_PROG_CXX that checks for cross
dnl C++ compilers
dnl ****************************************************************

AC_DEFUN(ACSPERT_TOOL_CXX,
[AC_BEFORE([$0], [AC_PROG_CXXCPP])dnl
ACSPERT_CHECK_TOOLS(CXX, $CCC c++ g++ gcc CC cxx, gcc)
AC_MSG_CHECKING(whether we are using GNU C++)
...
rm -f conftest*
  ...
  GXX=
  test "${CXXFLAGS+set}" = set || CXXFLAGS="-O"
fi
AC_MSG_CHECKING([default C++ compiler flags])
...
dnl ****************************************************************
dnl Work out the type of fltvec we want
dnl ****************************************************************

AC_DEFUN(ACSPERT_FLTVEC_TYPE,
[
acspert_cv_fltvec_type="FLTV_UNKNOWN"
if test "$host_cpu" = "torrent0"; then
  acspert_cv_fltvec_type="FLTV_SPERT0";
elif test "$host_cpu" = "sparc"; then
  acspert_cv_fltvec_type="FLTV_SUN4";
elif test "$host_cpu" = "mips"; then
  acspert_cv_fltvec_type="FLTV_IRIS";
elif test "$host_cpu" = "rs6000"; then
  acspert_cv_fltvec_type="FLTV_RS600";
fi
AC_MSG_CHECKING(which fltvec version to use)
AC_DEFINE_UNQUOTED($acspert_cv_fltvec_type)
AC_MSG_RESULT($acspert_cv_fltvec_type)
])

dnl The following macros are adapted from ~dpwe/share/lib/aclocal.m4
dnl and are used to setup SHLIB_CFLAGS and SHLIB_CXXFLAGS 
dnl so that files compiled with those flags will be suitable 
dnl for inclusion in shared libraries (*.so) further down 
dnl the line.
...
dnl $Header: /u/drspeech/repos/quicknet2/aclocal.m4,v 1.9 2004/11/04 19:14:14 davidj Exp $
dnl
dnl Clients:
