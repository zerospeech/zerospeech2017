const char* QN_SRIfeat_sparse_rcsid =
    "$Header: /u/drspeech/repos/quicknet2/QN_SRIfeat_sparse.cc,v 1.4 2010/10/29 02:43:31 davidj Exp $";

// This file contains routines for handling SRI sparse feature files. It is 
// based on QN_SRIfeat.cc.
...
const static char *sri_sparse_featurefile_type = "indexedfeaturefile";
...
void 
QN_InFtrStream_SRI_sparse::read_header(void) {
...
#define LBLEN 256
	...
	fprintf(stderr, "SRI Sparse Feat File Hdr: id of '%s' is not '%s' - lose\n", 
		lbuf, sri_hdr_string);
	return;
    }
    /* read header values */
	...
	*/
	...
	if(strcmp(tok, "end_head")==0)  {
      ...
      // must be of type indexedfeaturefile
 	    if (strcmp(tok, sri_sparse_featurefile_type)!=0) {
 	      log.error("Type of SRI sparse file %s is '%s' not '%s' - lose.", 
 			QN_FILE2NAME(file), tok, sri_sparse_featurefile_type);
	    }
	} else if(strcmp(tok, "feature_size")==0) {
	...
	/* hit eof before completing */
      log.error("Hit end of SRI sparse feat file %s before end_head.",
		QN_FILE2NAME(file));
    }
    if (data_offset == 0 || num_cols == 0 || total_frames == 0) {
      log.error("SRI sparse feat file %s did not include all required header fields.",
		QN_FILE2NAME(file));
    }
}
...
QN_InFtrStream_SRI_sparse::QN_InFtrStream_SRI_sparse(int a_debug,
    ...
    buffer(NULL)
{
	...
	log.error("Failed to seek to start of SRI sparse feat file '%s' header - "
    ...
    k_buffer = new int[num_cols];
    v_buffer = new float[num_cols];
    if (! (k_buffer && v_buffer )) {
	log.error("Unable to allocate memory for sparse key/value buffers.");
    }
    // Move to the start of the features
    rewind();
}

QN_InFtrStream_SRI_sparse::~QN_InFtrStream_SRI_sparse()
{
  delete[] buffer;
  delete[] k_buffer;
  delete[] v_buffer;
...
inline void
QN_InFtrStream_SRI_sparse::read_frame()
{
    int ec;			// Return code
    size_t numele_buffer;

    ec = fread(reinterpret_cast<char *>(&numele_buffer),sizeof(int),1,file);
    //size_t numele = htonl(numele_buffer);
    size_t numele = qn_htob_i32_i32(numele_buffer);
    if (ec!=1) {
      log.error("Failed to read number of elements in frame from SRI sparse file "
		             " '%s', frame=%lu file_offset=%lu.",
		         QN_FILE2NAME(file), current_frame, ftell(file));
	   }

    ec = fread(reinterpret_cast<char *>(v_buffer), 
	       sizeof(float), numele, file);
    if (ec!=numele) {
	       if (feof(file))	{ 
	          log.error("Failed to read sparse frame values from SRI sparse file "
		             " '%s', frame=%lu file_offset=%lu (%lx).  Received %d instead of %d.",
		         QN_FILE2NAME(file), current_frame, ftell(file),ftell(file),
           ec,numele);
        }
    }
    ec = fread(reinterpret_cast<char *>(k_buffer), 
	       sizeof(int), numele, file);
    if (ec!=numele) {
	       if (feof(file))	{ 
	          log.error("Failed to read sparse frame indices from SRI sparse file "
		             " '%s', frame=%lu file_offset=%lu.",
		         QN_FILE2NAME(file), current_frame, ftell(file));
        }
    }
    ...
    for (int j=0;j<numele;j++) {
        buffer[qn_htob_i32_i32(k_buffer[j])] = v_buffer[j];
    }
}

// Move to the start of the features
int
QN_InFtrStream_SRI_sparse::rewind()
{
    ...
    read_frame();
    log.log(QN_LOG_PER_EPOCH, "At start of SRI sparse feat file.");
    return 0;			// Should return senence ID
}

size_t
QN_InFtrStream_SRI_sparse::read_ftrs(size_t frames, float* ftrs)
{
    size_t count;		// Count of number of frames

    long first_frame = current_frame;

    for (count = 0; count<frames && current_frame < total_frames-1; count++) {
...
QN_SegID
QN_InFtrStream_SRI_sparse::nextseg()
{
...
QN_SegID
QN_InFtrStream_SRI_sparse::set_pos(size_t segno, size_t frameno)
{
...
int
QN_InFtrStream_SRI_sparse::get_pos(size_t* segnop, size_t* framenop)
{
...
size_t
QN_InFtrStream_SRI_sparse::num_frames(size_t segno)
{
...
QN_OutFtrStream_SRI_sparse::QN_OutFtrStream_SRI_sparse(int a_debug, 
                                         const char* a_dbgname,
                                         FILE* a_file, 
                                         size_t a_n_ftrs,
      ...
      header_pos(-1)
{
    ...
    k_buffer = new int[n_ftrs];
    v_buffer = new float[n_ftrs];
    clog.log(QN_LOG_PER_RUN, "Creating SRI file '%s'.",  QN_FILE2NAME(file) );
}

QN_OutFtrStream_SRI_sparse::~QN_OutFtrStream_SRI_sparse() {
  if (!bos) {
...
void QN_OutFtrStream_SRI_sparse::write_hdr(char* xtra_info) {
  ...
  */
  ...
  if (ftr_name!=NULL)  {
      sprintf(hdr_buff,"%s\nfile_type -s%lu %s",hdr_buff,
	      (unsigned long) strlen(sri_sparse_featurefile_type),sri_sparse_featurefile_type);
      sprintf(hdr_buff,"%s\nfeature_names -s%lu %s",hdr_buff,
	      (unsigned long) strlen(ftr_name),ftr_name);
  }
  else {
      sprintf(hdr_buff,"%s\nfile_type -s%lu %s",hdr_buff,
	      (unsigned long) strlen(sri_cepfile_type),sri_cepfile_type);
  }
  ...
  sprintf(hdr_buff,"%s\nfeature_size -i %lu",hdr_buff,(unsigned long) n_ftrs);
...
void QN_OutFtrStream_SRI_sparse::write_ftrs(size_t cnt, const float* ftrs)
{
    int ec;
    size_t i;
	...
	size_t j;
	size_t numele, numele_buffer;

	numele = 0;
	for (j=0;j<n_ftrs;j++) {
	    if (ftrs[j]!=0) {
		k_buffer[numele] = qn_btoh_i32_i32(j);
		v_buffer[numele] = buffer[j];
		numele++;
                if (numele>n_ftrs) {
                    clog.error("numele > n_ftrs (%lu)",n_ftrs);
                }
	    }
	}
	
	numele_buffer=qn_btoh_i32_i32(numele);

	ec = fwrite(&numele_buffer,sizeof(int),1,file);
	if (ec!=1)
	{
	    clog.error("Failed to write number of elements to SRI file '%s' - %s.",
		       QN_FILE2NAME(file), strerror(errno)); 
	}
	ec = fwrite(v_buffer, sizeof(float), numele, file);
	if (ec!=numele)
	{
	    clog.error("Failed to write frame to SRI file '%s' - %s.",
		       QN_FILE2NAME(file), strerror(errno)); 
	}
	ec = fwrite(k_buffer, sizeof(int), numele, file);
	if (ec!=numele)
	{
	    clog.error("Failed to write frame to SRI file '%s' - %s.",
		       QN_FILE2NAME(file), strerror(errno)); 
...
void QN_OutFtrStream_SRI_sparse::doneseg(QN_SegID segid) {
