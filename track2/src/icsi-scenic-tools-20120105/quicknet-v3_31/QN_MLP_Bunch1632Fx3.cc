const char* QN_MLP_Bunch1632Fx3_rcsid =
    "$Header: /u/drspeech/repos/quicknet2/QN_MLP_Bunch1632Fx3.cc,v 1.4 2011/03/03 00:32:58 davidj Exp $";
...
QN_MLP_Bunch1632Fx3::QN_MLP_Bunch1632Fx3(int dbg, size_t input,
					   size_t hidden,
					   size_t output,
					   QN_OutputLayerType outtype,
					   int a_in2hid_exp, int a_hid2out_exp,
					   size_t a_size_bunch)
    : clog(dbg, "QN_MLP_Bunch1632Fx3", ""),
      n_input(input),
      n_hidden(hidden),
      n_output(output),
      ...
      size_input(input * a_size_bunch),
      size_hidden(hidden * a_size_bunch),
      size_output(output * a_size_bunch),
      ...
      lr_sat_warning_given(0)
{
    ...
    hid_x32 = malloc_vi32(size_hidden);
    hid_x16 = malloc_vi16(size_hidden);
    hid_y = malloc_vi16(size_hidden);
    hid_sy = malloc_vi16(size_hidden);
    ...
    out_x32 = malloc_vi32(size_output);
    out_x16 = malloc_vi16(size_output);
    out_y = malloc_vi16(size_output);

    target16 = malloc_vi16(size_output);
    // These intermediate values are only needed for sigmoid output layers
    if (out_layer_type==QN_OUTPUT_SIGMOID)
    {
	out_dedy = malloc_vi16(size_output);
	out_dydx = malloc_vi16(size_output);
    }
    out_dedx = malloc_vi16(size_output);  
    qn_copy_i16_vi16(size_output, 0, out_dedx);
    hid2out_sdedx = malloc_vi16(size_output);
    qn_copy_i16_vi16(size_output, 0, hid2out_sdedx);
    in2hid_sdedx = malloc_vi16(size_output);
    qn_copy_i16_vi16(size_output, 0, in2hid_sdedx);

    neg_hid_learnrate = malloc_vi16(size_bunch);
    neg_out_learnrate = malloc_vi16(size_bunch);
    bias_update = malloc_vi32((n_output>n_hidden)?n_output:n_hidden);

    hid_dedy32 = malloc_vi32(size_hidden);
    hid_dedy16 = malloc_vi16(size_hidden);
    hid_dydx = malloc_vi16(size_hidden);
    hid_dedx = malloc_vi16(size_hidden);
    in2hid_sdedx = malloc_vi16(size_hidden);
...
QN_MLP_Bunch1632Fx3::~QN_MLP_Bunch1632Fx3()
{
    free_vi16(in_y);
    free_vi16(in2hid_h);
    free_vi16(in2hid_l);
...
size_t
QN_MLP_Bunch1632Fx3::size_layer(QN_LayerSelector layer) const
{
    size_t r;
...
size_t
QN_MLP_Bunch1632Fx3::num_connections() const
{
...
void
QN_MLP_Bunch1632Fx3::forward(size_t n_frames, const float* in, float* out)
{
    size_t i;
    size_t frames_this_bunch;
    ...
    {
        frames_this_bunch = min(size_bunch, n_frames-i);
...
void
QN_MLP_Bunch1632Fx3::train(size_t n_frames, const float* in,
...
void
QN_MLP_Bunch1632Fx3::forward_bunch(size_t n_frames,const float* in, float* out)
{
    fxbool sat;
    const size_t cur_size_input = n_input * n_frames;
...
#endif
    sat = conv_vf_vx16s(cur_size_input, in_y_exp, in, in_y);
#ifdef USE_IPM
...
#endif
    //  Layer1: matrix multiply for weights
    sat = mul_srx_mi16mi16_i_mi32t(n_frames,n_input,n_hidden,
				   n_input,n_hidden,n_hidden,
				   in_y,in2hid_h,
				   hid_x32_exp-in_y_exp-in2hid_exp-1,
				   hid_x32);
    ...
    IPM_timer_stop(&forw);
#endif
    //  Layer1: add in bias for each pattern
    fxint32* t_hid_x32 = hid_x32;
    for (i=0;i<n_frames;i++) {
      acc_vx32_vx32(n_hidden, hid_bias_exp, hid_x32_exp, hid_bias, t_hid_x32);
      t_hid_x32+=n_hidden;
    }

    //  Layer1: convert output to 16 bits
    conv_vx32_vx16(cur_size_hidden, hid_x32_exp, hid_x16_exp, hid_x32, hid_x16);
    //  Layer1: perform sigmoid
    sigmoid_vx16_vx16(cur_size_hidden, hid_x16_exp, hid_y_exp, hid_x16, hid_y);

    // Second layer
#ifdef USE_IPM
    IPM_timer_start(&forw);
#endif
    //  Layer2: matrix multiply for weights
    sat = mul_srx_mi16mi16_i_mi32t(n_frames,n_hidden,n_output,
				   n_hidden,n_output,n_output,
				   hid_y,hid2out_h,
				   out_x32_exp-hid_y_exp-hid2out_exp-1,
				   out_x32);
    ...
    IPM_timer_stop(&forw);
#endif
    //  Layer2: add in bias for each pattern (moved inside non-linearity)
    fxint32* t_out_x32 = out_x32;
    fxint16* t_out_y = out_y;
    
    //  Layer2: convert output to 16 bits
    //  Layer2: do output non-linearity
    switch(out_layer_type)
    {
    case QN_OUTPUT_SIGMOID:
    case QN_OUTPUT_SIGMOID_XENTROPY:
      // add biases
      for(i=0;i<n_frames;i++) {
	acc_vx32_vx32(n_output, out_bias_exp, out_x32_exp, out_bias, t_out_x32);
	t_out_x32 += n_output;
      }
      // convert 32->16
      conv_vx32_vx16(cur_size_output, out_x32_exp, out_x16_exp, out_x32, out_x16);
      // do sigmoid
      sigmoid_vx16_vx16(cur_size_output, out_x16_exp, out_y_exp, out_x16, out_y);
      break;
    case QN_OUTPUT_SOFTMAX:
      // add biases, don't bother to convert, and only do softmax on 1 pattern
      for(i=0;i<n_frames;i++) {
	acc_vx32_vx32(n_output, out_bias_exp, out_x32_exp, out_bias, t_out_x32);
	softmax_vx32_vx16(n_output, out_x32_exp, out_y_exp, t_out_x32, t_out_y);
	t_out_x32 += n_output;
	t_out_y += n_output;
      }
      break;
	...
	t_out_x32 += n_output;
      ...
      conv_vx32_vx16(cur_size_output, out_x32_exp, out_x16_exp, out_x32, out_x16);
      // do linear
      conv_vx16_vx16(cur_size_output, out_x16_exp, out_y_exp, out_x16, out_y);
      break;
    default:
	assert(0);
    }
    ...
    conv_vx16_vf(cur_size_output, out_y_exp, out_y, out);
#ifdef USE_IPM
...
void
QN_MLP_Bunch1632Fx3::train_bunch(size_t n_frames,const float *in, const float* target, float* out)
{
    fxbool sat;
    const size_t cur_size_hidden = n_hidden * n_frames;
    const size_t cur_size_output = n_output * n_frames;

    //clog.warning("forward pass");
// First move forward
    forward_bunch(n_frames,in, out);

// Find the error and propogate back through output layer in an appropriate
// manner 
    //clog.warning("convert targets");

#ifdef USE_IPM
    IPM_timer_start(&tofix);
#endif
    sat = conv_vf_vx16s(cur_size_output, target16_exp, target, target16);
#ifdef USE_IPM
    IPM_timer_stop(&tofix);
#endif
    //clog.warning("compute de/dx");
    switch(out_layer_type)
    {
    case QN_OUTPUT_SIGMOID:
	// For a sigmoid layer, de/dx = de/dy . dy/dx
	sub_vx16vx16_vx16(cur_size_output, out_y_exp, target16_exp, out_dedx_exp, out_y, target16, out_dedy);
	dsigmoid_vx16_vx16(cur_size_output, out_y_exp, out_dydx_exp, out_y, out_dydx);
	mul_vx16vx16_vx16(cur_size_output, out_dydx_exp, out_dedy_exp, out_dedx_exp,
	...
	sub_vx16vx16_vx16(cur_size_output, out_y_exp, target16_exp, out_dedx_exp, out_y, target16, out_dedx);
	break;
    default:
	assert(0);
    }
...
// Back propagate error through hidden to output weights
    ...
    //clog.warning("backprop through hiddent to output");

    sat = mulnt_srx_mi16mi16_i_mi32t(n_frames, n_output, n_hidden,
				     n_output, n_output, n_hidden,
				     out_dedx, hid2out_h,
				     -out_dedx_exp-hid2out_exp+hid_dedy32_exp-1,
				     hid_dedy32);
    if (sat!=FXFALSE)
    {
    ...
    IPM_timer_stop(&back);
#endif
    //clog.warning("conv hid_dedy");
    
    conv_vx32_vx16(cur_size_hidden, hid_dedy32_exp, hid_dedy16_exp, hid_dedy32,
		   hid_dedy16);

    //clog.warning("scale out_dedx");

  // scale out_dedx
    fxexp_t hid2out_sdedx_exp = out_y_exp + neg_hid2out_learnrate_exp;
    mul_vx16x16_vx16(cur_size_output,
                     out_dedx_exp, neg_hid2out_learnrate_exp,hid2out_sdedx_exp,
                     out_dedx, neg_hid2out_learnrate,hid2out_sdedx);
...
// Update hid2out weights and biases
    //mulacc_vx16x16_vx32(cur_size_output, out_dedx_exp, neg_out_learnrate_exp,
    // out_bias_exp, out_dedx, neg_out_learnrate, out_bias);
    // output biases first
    //clog.warning("update hid2out biases");
    sat = mul_vx16mx16_vx32t( n_frames, n_output,
                              neg_out_learnrate_exp,out_dedx_exp,out_bias_exp,
                              neg_out_learnrate, out_dedx, bias_update);
    satadd_vi32vi32_vi32( n_output, bias_update, out_bias, out_bias);

    //clog.warning("update hid2out weights");
    // then weights
    sat = multn_srx_srx_satadd_mi16mi16_i_i_mhi32_mhi32st(
                                 n_hidden, n_frames, n_output,
                                 n_hidden, n_output, n_output, n_output,
                                 hid_y, hid2out_sdedx,
                                 -hid_y_exp-hid2out_sdedx_exp+hid2out_exp-1,0,
                                 hid2out_h, hid2out_l, hid2out_h, hid2out_l);
    //fxexp_t hid_sy_exp = hid_y_exp + neg_hid2out_learnrate_exp;
    //mul_vx16x16_vx16(cur_size_hidden, hid_y_exp, neg_hid2out_learnrate_exp,
    ...
    //clog.warning("propagate error through sigmoid");
// Propagate error back through sigmoid
    dsigmoid_vx16_vx16(cur_size_hidden, hid_y_exp, hid_dydx_exp, hid_y, hid_dydx);
    mul_vx16vx16_vx16(cur_size_hidden, hid_dydx_exp, hid_dedy16_exp, hid_dedx_exp,
		      hid_dydx, hid_dedy16, hid_dedx);

    //clog.warning("scale hid_dedx");
  // scale hid_dedx
    fxexp_t in2hid_sdedx_exp = hid_dedx_exp + neg_in2hid_learnrate_exp;
    mul_vx16x16_vx16(size_hidden,
                     hid_dedx_exp, neg_in2hid_learnrate_exp, in2hid_sdedx_exp,
                     hid_dedx, neg_in2hid_learnrate, in2hid_sdedx);
...
// Update in2hid weights and biases
    ...
    //clog.warning("update in2hid biases");
    sat = mul_vx16mx16_vx32t( n_frames, n_hidden,
                              neg_hid_learnrate_exp,hid_dedx_exp, hid_bias_exp,
                              neg_hid_learnrate, hid_dedx, bias_update);
    satadd_vi32vi32_vi32( n_hidden, bias_update, hid_bias, hid_bias); 
    ...
    //clog.warning("update in2hid weights");
    sat = multn_srx_srx_satadd_mi16mi16_i_i_mhi32_mhi32st(
                                 n_input, n_frames, n_hidden,
                                 n_input, n_hidden, n_hidden, n_hidden,
                                 in_y, in2hid_sdedx,
                                 -in_y_exp-in2hid_sdedx_exp+in2hid_exp-1,0,
                                 in2hid_h, in2hid_l, in2hid_h, in2hid_l);
#ifdef USE_IPM
    IPM_timer_stop(&wup);
#endif
...
void
QN_MLP_Bunch1632Fx3::set_weights(enum QN_SectionSelector which, 
...
void 
QN_MLP_Bunch1632Fx3::get_weights(enum QN_SectionSelector which,
			      size_t row, size_t col,
			      size_t n_rows, size_t n_cols,
			      float* weights)
{
...
void
QN_MLP_Bunch1632Fx3::set_learnrate(enum QN_SectionSelector which, float learnrate)
{
    fxbool sat = FXFALSE;
	...
	sat = conv_f_x16s(neg_hid_learnrate_exp, -learnrate,
			  &lneg_hid_learnrate);
	qn_copy_i16_vi16(size_bunch,lneg_hid_learnrate,neg_hid_learnrate);
	clog.log(QN_LOG_PER_EPOCH,"Set hidbias learning rate %f (exp %i).",
		 -conv_x16_f(neg_hid_learnrate_exp, lneg_hid_learnrate),
		 neg_hid_learnrate_exp);
	break;
    case QN_MLP3_OUTPUTBIAS:
      fxint16 lneg_out_learnrate;
	...
	sat = conv_f_x16s(neg_out_learnrate_exp, -learnrate,
			  &lneg_out_learnrate);
	qn_copy_i16_vi16(size_bunch,lneg_out_learnrate,neg_out_learnrate);
	clog.log(QN_LOG_PER_EPOCH, "Set outbias learning rate %f (exp %i).",
		 -conv_x16_f(neg_out_learnrate_exp, lneg_out_learnrate),
...
float 
QN_MLP_Bunch1632Fx3::get_learnrate(enum QN_SectionSelector which) const
{
	...
	break;
    case QN_MLP3_HIDDENBIAS:
	res = -conv_x16_f(neg_hid_learnrate_exp, neg_hid_learnrate[0]);
	break;
    case QN_MLP3_OUTPUTBIAS:
	res = -conv_x16_f(neg_out_learnrate_exp, neg_out_learnrate[0]);
	break;
    default:
	res = 0;
