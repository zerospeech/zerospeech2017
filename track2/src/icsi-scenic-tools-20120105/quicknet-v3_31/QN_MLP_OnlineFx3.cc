const char* QN_MLP_OnlineFx3_rcsid =
    "$Header: /u/drspeech/repos/quicknet2/QN_MLP_OnlineFx3.cc,v 1.30 2004/09/07 23:30:01 davidj Exp $";
...
#include "QN_types.h"
#include "QN_Logger.h"
#include "QN_MLP_OnlineFx3.h"

QN_MLP_OnlineFx3::QN_MLP_OnlineFx3(int dbg, size_t input, size_t hidden,
				   size_t output, QN_OutputLayerType outtype,
				   int a_in2hid_exp, int a_hid2out_exp)
    : clog(dbg, "QN_MLP_OnlineFx3", ""),
      n_input(input),
      n_hidden(hidden),
      n_output(output),
      ...
      lr_sat_warning_given(0)
{
    ...
    in2hid = malloc_vi16(n_in2hid); qn_copy_i16_vi16(n_in2hid, 0, in2hid);
    ...
    hid2out = malloc_vi16(n_hid2out); qn_copy_i16_vi16(n_hid2out, 0, hid2out);
...
QN_MLP_OnlineFx3::~QN_MLP_OnlineFx3()
{
    free_vi16(in_y);
    free_vi16(in2hid);
    free_vi32(hid_bias);
...
size_t
QN_MLP_OnlineFx3::size_layer(QN_LayerSelector layer) const
{
    size_t r;
...
void
QN_MLP_OnlineFx3::size_section(QN_SectionSelector section, size_t* output_p,
...
size_t
QN_MLP_OnlineFx3::num_connections() const
{
...
void
QN_MLP_OnlineFx3::forward(size_t n_frames, const float* in, float* out)
{
    size_t i;
...
void
QN_MLP_OnlineFx3::train(size_t n_frames, const float* in, const float* target,
			float* out)
{
    size_t i;
...
void
QN_MLP_OnlineFx3::forward1(const float* in, float* out)
{
    fxbool sat;
    ...
    sat = mul_vx16mx16_vx32t(n_input, n_hidden, in_y_exp, in2hid_exp,
			     hid_x32_exp, in_y, in2hid, hid_x32);
    if (sat!=FXFALSE)
    {
    ...
    sat = mul_vx16mx16_vx32t(n_hidden, n_output, hid_y_exp, hid2out_exp,
			     out_x32_exp, hid_y, hid2out, out_x32);
    if (sat!=FXFALSE)
    {
...
void
QN_MLP_OnlineFx3::train1(const float *in, const float* target, float* out)
{
    fxbool sat;
...
// Back propagate error through hidden to output weights
    ...
    sat = mul_mx16vx16_vx32t(n_hidden, n_output, hid2out_exp, out_dedx_exp,
			     hid_dedy32_exp, hid2out, out_dedx, hid_dedy32);
    if (sat!=FXFALSE)
    {
...
// Update hid2out weights and biases
    ...
    IPM_timer_start(&wup);
#endif
    mulacc_vx16vx16_mx16s(n_hidden, n_output, hid_sy_exp, out_dedx_exp,
			  hid2out_exp, hid_sy, out_dedx, hid2out);
#ifdef USE_IPM
    IPM_timer_stop(&wup);
#endif
...
// Update in2hid weights and biases
    ...
    IPM_timer_start(&wup);
#endif
    mulacc_vx16vx16_mx16s(n_input, n_hidden, in_y_exp, in2hid_sdedx_exp,
			  in2hid_exp, in_y, in2hid_sdedx, in2hid);
#ifdef USE_IPM
    IPM_timer_stop(&wup);
#endif
...
void
QN_MLP_OnlineFx3::set_weights(enum QN_SectionSelector which, 
			      ...
			      const float* weights)
{
    ...
    {
	const float* src_ptr = weights;
	fxint16* dest_ptr = hid2out + col*n_output + row;
	fxint16* buf = falloc_vi16(n_cols);
	for (size_t i=0; i<n_rows; i++)
	{
	    weight_sat |= conv_vf_vx16s(n_cols, hid2out_exp, src_ptr, buf);
	    copy_vi16_svi16(n_cols, n_output, buf, dest_ptr);
	    src_ptr += n_cols;
	    dest_ptr += 1;
	}
	ffree_vi16(buf);
...
void 
QN_MLP_OnlineFx3::get_weights(enum QN_SectionSelector which,
			      size_t row, size_t col,
			      size_t n_rows, size_t n_cols,
			      float* weights) const
{
	...
	fxint16* src_ptr = in2hid + col*n_hidden + row;
	fxint16* buf = falloc_vi16(n_cols);
	for (size_t i=0; i<n_rows; i++)
	{
    ...
    {
	float* dest_ptr = weights;
	fxint16* src_ptr = hid2out + col*n_output + row;
	fxint16* buf = falloc_vi16(n_cols);
	for (size_t i=0; i<n_rows; i++)
	{
...
void
QN_MLP_OnlineFx3::set_learnrate(enum QN_SectionSelector which, float learnrate)
{
    fxbool sat = FXFALSE;
...
float 
QN_MLP_OnlineFx3::get_learnrate(enum QN_SectionSelector which) const
{
