#ifndef NO_RCSID
const char *QN_Range_rcsid =
  "$Header: /u/drspeech/repos/quicknet2/QN_Range.cc,v 1.6 2010/10/29 02:43:30 davidj Exp $";
#endif
//
// QN_Range.cc
//
...
QN_RangeNode** FindLastRangeList(QN_RangeNode** last) {
    while(*last != NULL) {last = &((*last)->next); }
    return last;
}

int QN_RangeNode::SubtractVal(int val) {
	...
	if (val == start) {
	    ...
	    next = new QN_RangeNode(next, val + step, end, step);
	    end = val - step;
	}
    }
    return (n_pts() > 0);
...
int QN_RangeNode::SubtractRange(QN_RangeNode *sub) {
	...
	if ( (start%step) == (sub->start % step) ) {
		...
		next = new QN_RangeNode(next, start+step*(end_index+1), end,step);
...
void FreeRangeList(QN_RangeList list) {
    QN_RangeNode *doomed;
    while(list) {
	doomed = list;
	list = doomed->next;
...
int CanonicalRangeList(QN_RangeList *plist) {
    // Remove null nodes, possibly merge adjacent nodes
    QN_RangeNode *cur, *nxt;
...
const char *QN_Range::colons = ":-";   // valid separators for matlab range specs
const char *QN_Range::commas = ", \t"; // valid separators for number lists
const char *QN_Range::slashes = "/";   // valid separators of exclude lists
const char *QN_Range::semis  = ";\n";  // valid top-level term separators
const char *QN_Range::space = " \t";   // non-linebreak WS
const char *QN_Range::spacebreak = " \t\n\r";  // any WS
const char *QN_Range::alpha = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz_";


int QN_Range::_parseNumber(int *pval) {
...
int QN_Range::_parseMatlabRange(int *pstart, int *pstep, int *pend) {
...
int QN_Range::_parseRange(int *pstart, int *pstep, int *pend) {
...
int QN_Range::_parseSequence(QN_RangeList *seq) {
	...
	if ( (rc=_parseRange(&start, &step, &end)) < 0) {
	    ...
	    *seq = new QN_RangeNode(*seq, start, end, step);
	    seq = &((*seq)->next);
...
int QN_Range::_parseTerm(QN_RangeList *seq) {
    // Parse the full SEQUENCE [/SEQUENCE] syntax
    QN_RangeList positive = NULL, negative = NULL;
    int rc;
    ...
    _subtractSeqs(&positive, negative);
    *seq = positive;
...
int QN_Range::_parseSpec(QN_RangeList *seq) {
...
int QN_Range::_compileDefStr(void) {
...
int QN_Range::_compileSpecFile(char *filename) {
	...
	fprintf(stderr, "QN_Range::_compileFileSpec: can't read '%s'\n", 
		filename);
	goto exit;
    }
...
void QN_Range::_reportParseError(char *def_str) {
    // Print a report of the parse error using built-in data
    fprintf(stderr, "QN_Range::Parse error: %s\n", errmsg);
    fprintf(stderr, ">> %s\n", def_str);
    char fmtstr[32];
    sprintf(fmtstr, ">> %%%lds\n", (long) (pos-def_str));
...
void QN_Range::PrintRanges(char *tag/*=NULL*/, FILE *stream/*=stderr*/) {
    // For debug, print out the actual ranges specified
    QN_RangeNode *node = rangeList;
...
unsigned int QN_Range::length(void) {
    // Total number of values specified by a list
    unsigned int size = 0;
    QN_RangeList list = rangeList;
    while(list) {
	size += list->n_pts();
	list = list->next;
    }
...
int QN_Range::_subtractSeqs(QN_RangeList *ppos, QN_RangeList neg) {
    // Go through making the subtractions from a range list
    QN_RangeNode **pnextpos;
    QN_RangeNode *doomed;
...
int QN_Range::SetLimits(int minval/*=0*/, int ulimval/*=QN_RNG_VAL_BAD*/) {
...
int QN_Range::SetDefStr(const char *spec) {
	...
	if (def_string[0] == '@') {
	    /* type = QN_RNG_TYPE_FILE; */
	    if ( (rc = _compileSpecFile(def_string+1)) > 0) {
		type = QN_RNG_TYPE_LIST;
	    }	    
	} else {
...
QN_Range::QN_Range(const char *defstr /*=NULL*/, int minval /*=0*/, 
	     int ulimval /*=QN_RNG_VAL_BAD*/) {
    def_string = NULL;
    rangeList = NULL;
    type = QN_RNG_TYPE_NONE;
    min_val=0; max_val=0;
...
QN_Range::iterator::iterator (const QN_Range& rng) 
    : filename(0), file_handle(0)   // implicit, but pleases purify
{
    reset(rng);
}

QN_Range::iterator::iterator (const iterator& it) {
...
QN_Range::iterator::~iterator(void) {
    if (file_handle != NULL) {
	fclose(file_handle);
	file_handle = NULL;
    }
...
int QN_Range::iterator::read_next (int *result, FILE *file_handle, const char* filename) {
...
int QN_Range::iterator::reset (void) {
    return reset(*myrange);
}

int QN_Range::iterator::reset (const QN_Range& rng) {
    cur_node = NULL;
    file_handle = NULL;
    atEnd = 0;
    myrange = &rng;
    if (myrange->GetType() == QN_RNG_TYPE_FILE) {
	filename = myrange->GetDefStr()+1;
	if ( (file_handle = fopen(filename, "r")) == NULL) {
	    fprintf(stderr, "QN_Range::iterator: unable to read file '%s'\n", 
		    filename);
	    exit(-1);
	}
	buflen = 1024;
...
int QN_Range::iterator::step_by(int n) {
...
int QN_Range::iterator::next_el(void) {
    if (file_handle) {
	// Assume a file type
	if ( read_next(&cur_value, file_handle, filename) == 0) {
	    atEnd = 1;
...
int QN_Range::iterator::current_range_finite(void) const {
...
int max_val = QN_RNG_VAL_BAD;
int min_val = QN_RNG_VAL_BAD;
char *spec = NULL;
int indx = QN_RNG_VAL_BAD;
int debug = 0;
int quiet = 0;
int commas = 0;
int horizontal = 0;
...
main(int argc, char **argv) {
    ...
    QN_Range r(spec, min_val, max_val+1);
    if (debug) {
    ...
    if (indx != QN_RNG_VAL_BAD) {
	int sby=5;
	fprintf(stderr, "list[%d] = %d\n", indx, r.index(indx));
	QN_Range::iterator it = r.begin();
	int i;
	for (i = 0; i < indx; ++i) {
	    ++it;
	}
    ...
    if (!quiet) {
	int first = 1;
	for (QN_Range::iterator it = r.begin(); 
	     !it.at_end(); it++) {
