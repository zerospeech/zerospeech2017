/***************************************************************\
*   sndfnist.c				Switchboard NIST fmt	*
...
char *sndfExtn = ".wav";		/* file extension, if used a*/
...
#define LINELEN 256
    ...
    if(nisthostbytemode < 0)	nisthostbytemode = HostByteMode();
	...
	if((bytemode = GetBytemode(file)) == -1) {
	    ...
	    bytemode = nisthostbytemode;
	    SetBytemode(file, bytemode);
	}
	if((nisthostbytemode == BM_INORDER && bytemode == BM_INORDER) ||
	    (nisthostbytemode != BM_INORDER && bytemode != BM_INORDER)) {
	    t = "10";
	} else {
