...
Segment *readseglist_file(char *fn, int *N) 
{
   int Nsegs = file_line_count( fn );

   Segment *segs = (Segment *) MALLOC( Nsegs*sizeof(Segment) );
   
   FILE *fptr = fopen(fn, "r");
   int wcnt = 0;
   while ( wcnt < Nsegs && 
	   fscanf(fptr, "%s%d%d",
		  segs[wcnt].seglabel, &segs[wcnt].xA, &segs[wcnt].xB ) != EOF ) {
      segs[wcnt].xA--;
      segs[wcnt].xB--;
      //fprintf(stderr, "%s %d %d\n", segs[wcnt].seglabel, segs[wcnt].xA, segs[wcnt].xB);
      wcnt++;
   }
   fclose(fptr);
